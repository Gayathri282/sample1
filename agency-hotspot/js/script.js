...
$("#team-slider").on("click", ".owl-item", function(e){
    var $carousel = $("#team-slider");
...
document.addEventListener('DOMContentLoaded', function () {
    var portfolioListItems = document.querySelectorAll('.portfolio-list li');
    var currentIndex = 0;
    ...
    function highlightItem(index) {
        ...
        portfolioListItems[index].classList.add('active');
    }

    function updateActiveClass() {
        // Increment the index (loop back to 0 if it exceeds the item count)
        currentIndex = (currentIndex + 1) % portfolioListItems.length;

        // Highlight the current item
        highlightItem(currentIndex);
    }

    // Start the highlighting animation by calling the function
    setInterval(updateActiveClass, 2000); // Adjust the interval as needed

    // Event listener for clicking on a portfolio item
    portfolioListItems.forEach(function (item, index) {
        item.addEventListener('click', function () {
            ...
            // Set the currentIndex to the clickedIndex
            currentIndex = index;
        });
    });
});
